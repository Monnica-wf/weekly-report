...
class WeeklyReportApp {
    ...
    saveItem() {
        ...
        this.syncPlansToNextWeekTasks(this.currentYear, this.currentWeek);
    ...
    /**
     * 将某一周的“下周计划”同步为下一周的本周工作
     */
    syncPlansToNextWeekTasks(year, week) {
        const sourceWeekData = storage.getWeekData(year, week);
        const next = this.getAdjacentWeek(year, week, 1);
        const nextWeekData = storage.getWeekData(next.year, next.week);
        const sourceWeekId = storage.getWeekId(year, week);
        ...
        const pushGeneratedTask = (item, sourceType) => {
            if (!item?.title?.trim()) return;

            const syncKey = `${sourceWeekId}:${sourceType}:${item.id}`;
            generatedTasks.push({
                ...item,
                id: `plan-${syncKey}`,
                title: item.title,
                content: item.content || `来自第${week}周下周计划`,
                progress: item.progress !== undefined && sourceType === 'incomplete' ? item.progress : 0,
                endDate: null,
                milestones: [],
                fromMilestone: false,
                isFromMilestone: false,
        ...
        (sourceWeekData.harvests || []).forEach(plan => {
            pushGeneratedTask(plan, 'harvest');
        });

        (sourceWeekData.tasks || []).forEach(task => {
            const progress = task.progress !== undefined ? task.progress : 100;
            const isAutoGenerated = task.isSyncedPlanTask;
            if (progress < 100 && !isAutoGenerated) {
                pushGeneratedTask(task, 'incomplete');
            }
        });

        const existingManualTasks = (nextWeekData.tasks || []).filter(task => {
            if (!task.isSyncedPlanTask) return true;
            return task.sourceWeekId !== sourceWeekId;
        });

        const existingGeneratedByKey = {};
        (nextWeekData.tasks || []).forEach(task => {
            if (task.isSyncedPlanTask && task.syncKey) {
                existingGeneratedByKey[task.syncKey] = task;
            }
        });
    ...
    /**
     * 载入时修复旧数据：把历史下周计划同步到下一周工作
     */
    syncAllPlansToNextWeekTasks() {
        const allData = storage.getAllData();
        Object.keys(allData).forEach(weekId => {
            const match = weekId.match(/^(\d{4})-W(\d{2})$/);
            if (!match) return;
            ...
            const week = Number(match[2]);
            const weekData = allData[weekId];
            const hasPlans = (weekData.harvests || []).length > 0 ||
                (weekData.tasks || []).some(task => {
                    const progress = task.progress !== undefined ? task.progress : 100;
                    return progress < 100 && !task.isSyncedPlanTask;
                });

            if (hasPlans) {
                this.syncPlansToNextWeekTasks(year, week);
            }
        });
    }
